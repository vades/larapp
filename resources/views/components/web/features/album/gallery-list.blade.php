<x-web.layout :title="$page->metaTitle" :description="$page->metaDescription" :keywords="$page->keywords">
    ...
    <section  x-data="{ activeIndex: 0, items: $el.querySelectorAll('[data-carousel-item]'), showLightbox: true }"
        ...
        <article class="flex items-center gap-4 h-[60rem]" x-show="showLightbox">
        ...
        <div class="flex-1 flex items-center justify-center">
                ...
                @foreach(collect($images) as $item)
                    <figure class="hidden duration-700 ease-in-out" data-carousel-item  @click="items[activeIndex].classList.add('hidden'); activeIndex = (activeIndex + 1) % items.length; items[activeIndex].classList.remove('hidden')">
                        <img src="{{$item->src}}" class="max-w-full max-h-[80vh]" alt="{{ $item->title}}">
